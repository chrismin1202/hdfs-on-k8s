...
function configure_host_resolver() {
...
if [ "${HADOOP_TYPE}" == "master" ]; then
  hdfs --config "${HADOOP_HOME}" namenode -format -nonInteractive
  hdfs --config "${HADOOP_HOME}/etc/hadoop" namenode
else
  while [ "$response" != "200" ]
  do
